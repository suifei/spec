...
1. `00-hook` — 问题：需求散在聊天里、一路绿灯却说不清凭什么
2. `01-install` — 一行命令装好，只碰 `.claude/`
3. `02-usage` — 三个命令是三道门：`/spec` → `/build` → `/yolo`
4. `03-collab` — 人机协同：AI 扛重活，只有真分叉才问你；诚实说"不"
5. `04-capabilities` — 核心能力：探针必须能变红、Intent/Acceptance/Method、独立评审、文件系统即记忆
6. `05-proof` — 效果：自己管自己，22 条探针全绿；独立审计抓出自我夸大并逐条改掉

## Assets

- ../narration/00-hook.txt … ../narration/05-proof.txt — 六段中文旁白定稿，已按 NowVoice
...
- 视觉沿用仓库自己的 GitHub 暗色语汇（#0d1117 底、#3fb950 绿门、#f85149 红探针），
  让片子看起来就是这个工具的栖息地。
- `04-capabilities` 必须真实演一次"探针变红再变绿"——这是整个产品的核心主张，不能只用文字说。
