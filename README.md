...
Run one line **inside your project** to install `/spec` + `/build` into its
`.claude/`. It only ever touches `.claude/skills/{spec,build}/` and
`.claude/commands/{spec,build}.md` — never your `CLAUDE.md`, `SPEC.md`, or `.spec/`
(those are generated by `/spec` itself, and are yours).
...
virtually every Linux/macOS system) — no other dependency. Re-running upgrades
in place. Then open Claude Code in that directory and run `/spec`.
...
scripts/install.cmd                    # one-line installer (Windows cmd.exe)
