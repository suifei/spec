...
Run one line **inside your project** to install `/spec` + `/build` + `/yolo` into
its `.claude/`. It only touches `.claude/skills/{spec,build,yolo}/`,
`.claude/commands/{spec,build,yolo}.md`, and `.claude/spec-install.manifest` — never your `CLAUDE.md`, `SPEC.md`, or
`.spec/` (those are generated by `/spec` itself, and are yours).
