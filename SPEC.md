...
| G4 | construction progress is written back to the phases ledger | runnable meta-probe | if `STATE.md` `## build` reports built, `SPEC.md`'s phases ledger records **that phase's** construction (closes the S1-3 gap, incl. later phases) | `.spec/probes/G4-writeback-coherence.sh` (+ `--selftest`) | 2026-07-08 / vm | ✅ floor-checked (linter) |
